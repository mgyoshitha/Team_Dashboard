...
export function StatusBadge({ status }: { status: EmployeeStatus }) {
  return (
    <span className={cn('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize', employeeStatusStyles[status])}>
      {status.replace('_', ' ')}
    </span>
  );
}
...
export function ProjectStatusBadge({ status }: { status: ProjectStatus }) {
  return (
    <span className={cn('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize', projectStatusStyles[status])}>
      {status.replace('_', ' ')}
    </span>
  );
}

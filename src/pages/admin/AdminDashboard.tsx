...
import { getInitials, getAvatarColor, formatDate, cn, normalizeProjectStatus } from '@/lib/utils';
import {
...
export function AdminDashboard() {
  ...
  const projectsList = projects.filter((p) => (p.project_type || 'project') === 'project');
  const acceleratorsList = projects.filter((p) => (p.project_type || 'project') === 'accelerator');
  const pocsList = projects.filter((p) => (p.project_type || 'project') === 'poc');
  const allocatedEmployees = employees.filter((e) => getEmployeeAllocation(allocations, e.id) > 0);
  ...
  const statusData = [
    { name: 'Allocated', value: allocatedEmployees.length, fill: '#10B981' },
  ...
  const recentProjects = projects.filter((p) => (p.project_type || 'project') === 'project').slice(0, 4);
  const recentAccelerators = projects.filter((p) => (p.project_type || 'project') === 'accelerator').slice(0, 4);
  const recentPocs = projects.filter((p) => (p.project_type || 'project') === 'poc').slice(0, 4);
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie
                    ...
                    onClick={(entry) => {
                      if (entry?.id) {
                        navigate(`/admin/projects/${entry.id}`);
                      }
                    }}
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-3">
              {recentAccelerators.length === 0 ? (
                <p className="text-sm text-muted-foreground">No accelerators yet</p>
              ) : (
                recentAccelerators.map((p) => (
                  <Link key={p.id} to={`/admin/projects/${p.id}`} className="flex items-center justify-between rounded-lg border border-gray-100 p-3 hover:bg-gray-50">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-gray-900">{p.project_name}</p>
                      <p className="truncate text-xs text-muted-foreground">{p.client_name} · {formatDate(p.start_date)}</p>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-3">
              {recentPocs.length === 0 ? (
                <p className="text-sm text-muted-foreground">No POCs yet</p>
              ) : (
                recentPocs.map((p) => (
                  <Link key={p.id} to={`/admin/projects/${p.id}`} className="flex items-center justify-between rounded-lg border border-gray-100 p-3 hover:bg-gray-50">
                    <div className="min-w-0 flex-1">

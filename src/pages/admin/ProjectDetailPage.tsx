...
import { ArrowLeft, Calendar, User, Building2, FileText, Pencil } from 'lucide-react';
...
import { getInitials, getAvatarColor, formatDate, cn, normalizeProjectStatus, mapProjectStatusToStore } from '@/lib/utils';
import type { Project, Allocation, ProjectType } from '@/lib/types';
...
export function ProjectDetailPage() {
  ...
  const workflowStatus = normalizeProjectStatus(project.status);
  const activeAllocs = allocations.filter((a) => a.status === 'active');
  const projectTypeLabel = project.project_type === 'accelerator' ? 'Accelerator' : project.project_type === 'poc' ? 'POC' : 'Project';
  ...
  const handleStatusChange = async (status: string) => {
    const { error } = await supabase.from('projects').update({ status: mapProjectStatusToStore(status) }).eq('id', project.id);
    if (error) {
      console.error(error);
      return;
    }
    setProject((prev) => prev ? { ...prev, status: normalizeProjectStatus(status) } : prev);
  };
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <ProjectStatusBadge status={workflowStatus} />
        ...
        <div className="space-y-6">
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-3">
              ...
              <InfoRow icon={<User className="h-4 w-4" />} label="Type" value={projectTypeLabel} />
              ...
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-sm text-muted-foreground"><FileText className="h-4 w-4" />Workflow Status</span>
                <Select value={workflowStatus} onValueChange={handleStatusChange}>
      ...
      <CreateProjectDialog open={editOpen} onOpenChange={setEditOpen} project={project} defaultProjectType={(project.project_type || 'project') as ProjectType} onSuccess={(updatedProject) => setProject(updatedProject)} />
    </div>
  );
}
